...
int MakeList(HWND hWnd)
{
	...
	for (int i = 0, j = 0; i < MAX_SEARCH; i++) {
		...
		if (
			strtoul(szId, NULL, 16) != 0 &&
			(
			(isupper(szName[0]) != 0 && islower(szName[1]) != 0) ||
				strcmp(szName, _T("???")) == 0 ||
				(strncmp(szName, _T(" "), 1) == 0 && isupper(szName[1]) && islower(szName[2]))
				)
			) {
...
int CALLBACK CmpProc(LPARAM lp1,LPARAM lp2,LPARAM lp3)
{
	...
	//"-"の時は一番下にする 0.57で追加
	if(wcscmp(szBuf1,_T("-"))==0){
		if (g_Sort[(int)lp3] == UP)sprintf(szBuf1, _T("ZZZZZZZZZZ"));
		else sprintf(szBuf1, _T(""));
	}
	else if (wcslen(szBuf1) == 0) {//空白 0.57aで追加
		if (g_Sort[(int)lp3] == UP)sprintf(szBuf1, _T("ZZZZZZZZZZZZ"));
	}
	if (wcscmp(szBuf2, _T("-")) == 0) {
		if (g_Sort[(int)lp3] == UP)sprintf(szBuf2, _T("ZZZZZZZZZZ"));
		else sprintf(szBuf2, _T(""));
	}
	else if (wcslen(szBuf2) == 0) {//空白 0.57aで追加
		if (g_Sort[(int)lp3] == UP)sprintf(szBuf2, _T("ZZZZZZZZZZZZ"));
	}
...
#define	WAR_OUT	(1<<11)
	...
	if (nNowChecked != snPastChecked) {
		for (int i = 0; i < nItemCount; i++) {
			...
			if (ListView_GetCheckState(hLIST, i) == 0) {//未チェックの時
				...
				nCheckedID = strtoul(szId, NULL, 16);
	...
	for (int i = 0; i < nItemCount; i++) {
		if (ListView_GetCheckState(hLIST, i) != 0) {
			...
			if (strcmp(szType, _T("EX")) == 0) {//空は通常、EXはEXサウンド //0.47で変更
				nowState.sound = 1;
			}
			...
			if (target.inrange != 0) {//範囲内
				if (NPC_CORPSE(target)) {//ST_CORPSE
					...
					if (target.x == 0 && target.y == 0 && target.z == 0) {
						...
						case ST_POP:
							...
							wsprintf(szSendText, _T("%-24s POP->WAR    code=%d id=%03X hpp=%02X inrange=%02X state=%02X\n"), target.name, target.code, target.id, target.hpp, target.inrange, target.state);
							break;
						case ST_CORPSE:
							uSoundType[nowState.sound] |= CORPSE_WAR;//0.49で変更
							wsprintf(szSendText, _T("%-24s CORPSE->WAR code=%d id=%03X hpp=%02X inrange=%02X state=%02X\n"), target.name, target.code, target.id, target.hpp, target.inrange, target.state);
							break;
						case ST_OUT://0.60で変更
							uSoundType[nowState.sound] |= OUT_WAR;//0.49で変更
							wsprintf(szSendText, _T("%-24s OUT->WAR    code=%d id=%03X hpp=%02X inrange=%02X state=%02X\n"), target.name, target.code, target.id, target.hpp, target.inrange, target.state);
							break;
						default://0.60で変更
							uSoundType[nowState.sound] |= OUT_WAR;
							wsprintf(szSendText, _T("%-24s NON->WAR    code=%d id=%03X hpp=%02X inrange=%02X state=%02X\n"), target.name, target.code, target.id, target.hpp, target.inrange, target.state);
							break;
						}
					}
				}
					...
					if (target.x == 0 && target.y == 0 && target.z == 0) {
						...
						case ST_WAR:
							...
							wsprintf(szSendText, _T("%-24s WAR->POP    code=%d id=%03X hpp=%02X inrange=%02X state=%02X\n"), target.name, target.code, target.id, target.hpp, target.inrange, target.state);
							break;
						case ST_POP:
							break;
						case ST_CORPSE:
							uSoundType[nowState.sound] |= CORPSE_POP;//0.49で変更
							wsprintf(szSendText, _T("%-24s CORPSE->POP code=%d id=%03X hpp=%02X inrange=%02X state=%02X\n"), target.name, target.code, target.id, target.hpp, target.inrange, target.state);
							break;
						case ST_OUT://0.60で変更
							uSoundType[nowState.sound] |= OUT_POP;//0.49で変更
							wsprintf(szSendText, _T("%-24s OUT->POP    code=%d id=%03X hpp=%02X inrange=%02X state=%02X\n"), target.name, target.code, target.id, target.hpp, target.inrange, target.state);
							break;
						default://0.60で変更
							uSoundType[nowState.sound] |= OUT_POP;
							wsprintf(szSendText, _T("%-24s NOC->POP    code=%d id=%03X hpp=%02X inrange=%02X state=%02X\n"), target.name, target.code, target.id, target.hpp, target.inrange, target.state);
							break;
						}
					}
				}
				...
				if (target.x == 0 && target.y == 0 && target.z == 0) {
					...
					case ST_WAR:
						...
						wsprintf(szSendText, _T("%-24s WAR->OUT    code=%d id=%03X hpp=%02X inrange=%02X state=%02X\n"), target.name, target.code, target.id, target.hpp, target.inrange, target.state);
						break;
					case ST_POP:
						uSoundType[nowState.sound] |= POP_OUT;//0.49で変更
						wsprintf(szSendText, _T("%-24s POP->OUT    code=%d id=%03X hpp=%02X inrange=%02X state=%02X\n"), target.name, target.code, target.id, target.hpp, target.inrange, target.state);
						break;
					case ST_CORPSE:
						uSoundType[nowState.sound] |= CORPSE_OUT;//0.49で変更
						wsprintf(szSendText, _T("%-24s CORPSE->OUT code=%d id=%03X hpp=%02X inrange=%02X state=%02X\n"), target.name, target.code, target.id, target.hpp, target.inrange, target.state);
						break;
					case ST_OUT://0.60で変更
						break;
					default://0.60で変更
						wsprintf(szSendText, _T("%-24s NON->OUT    code=%d id=%03X hpp=%02X inrange=%02X state=%02X\n"), target.name, target.code, target.id, target.hpp, target.inrange, target.state);
						break;
					}
				}
			}
			...
			if (strcmp(szState, item.pszText) != 0) {
				if (FALSE == ListView_SetItem(hLIST, &item)) {
					return 5;
				}
			}
			...
			if (target.inrange) {
				...
				if (NPC_CORPSE(target) || fDistance > 60.0) {//0.57aで変更
					wsprintf(szDistance, _T("-"));//ST_CORPSE
				}
				else if (fDistance >= 10.0)sprintf(szDistance, _T("%.1f"), fDistance);
				else sprintf(szDistance, _T("0%.1f"), fDistance);//なぜか"%02.1f"が利用出来ないため
			}
			else wsprintf(szDistance, _T("-"));
			if (FALSE == ListView_SetItem(hLIST, &item)) {
				return 8;
			}
			...
			if (pastState[nCheckedID].time != 0) {
				if (nowtime - pastState[nCheckedID].time == 0) {//0.42で追加00:00の時表示しない
					...
					if (tHour != 0) {
						sprintf(szTime, _T("%d:%02d:%02d"), tHour, tMin, tSec);
					}
					else {
						sprintf(szTime, _T("%02d:%02d"), tMin, tSec);
					}
...
int List_ReadNPC(HWND hWnd,HWND hList,int iFlagAutoLoad)
{
	...
	if (AreaID >= 0x0100) {
		//セクション名を作成
		wsprintf(szSection, _T("Area%04X"), AreaID);
		//フィルターを作成
		ZeroMemory(szFilter, sizeof(szFilter));
		if (g_Menu.list_control == 0) {
			//ファイルネーム作成
			wsprintf(szFullPathName, _T("AutoLoad.Area%04X"), AreaID);
			//\0を_で作成しているため、後で置換
			wsprintf(szFilter, _T("Area%04X File(*.Area%04X)_*.Area%04X_All (*.*)_*.*__"), AreaID, AreaID, AreaID);
		}
		else {
			//ファイルネーム作成
			wsprintf(szFullPathName, _T("AutoLoad.Area%04X-Name"), AreaID);
			//\0を_で作成しているため、後で置換
			wsprintf(szFilter, _T("Area%04X File(*.Area%04X-Name)_*.Area%04X-Name_All (*.*)_*.*__"), AreaID, AreaID, AreaID);
		}
	}
	else {
		//セクション名を作成
		wsprintf(szSection, _T("Area%02X"), AreaID);
		//フィルターを作成
		ZeroMemory(szFilter, sizeof(szFilter));
		if (g_Menu.list_control == 0) {
			//ファイルネーム作成
			wsprintf(szFullPathName, _T("AutoLoad.Area%02X"), AreaID);
			//\0を_で作成しているため、後で置換
			wsprintf(szFilter, _T("Area%02X File(*.Area%02X)_*.Area%02X_All (*.*)_*.*__"), AreaID, AreaID, AreaID);
		}
		else {
			//ファイルネーム作成
			wsprintf(szFullPathName, _T("AutoLoad.Area%02X-Name"), AreaID);
			//\0を_で作成しているため、後で置換
			wsprintf(szFilter, _T("Area%02X File(*.Area%02X-Name)_*.Area%02X-Name_All (*.*)_*.*__"), AreaID, AreaID, AreaID);
		}
	}
	...
	if(iFlagAutoLoad == 0){
		...
		if (AreaID >= 0x0100) {
			if (g_Menu.list_control == 0) {
				wsprintf(szFileName, _T("%s\\AutoLoad.Area%04X"), g_NpcListPath, AreaID);
			}
			else {
				wsprintf(szFileName, _T("%s\\AutoLoad.Area%04X-Name"), g_NpcListPath, AreaID);
			}
		}
		else {
			if (g_Menu.list_control == 0) {
				wsprintf(szFileName, _T("%s\\AutoLoad.Area%02X"), g_NpcListPath, AreaID);
			}
			else {
				wsprintf(szFileName, _T("%s\\AutoLoad.Area%02X-Name"), g_NpcListPath, AreaID);
			}
		}
	...
	sprintf(szReplaceFileName, _T("%s\\ReplaceName.ini"), szCurrentDir);
	...
	if (g_Menu.list_control == 0) {
		...
		for (int i = 1; i <= nItemCount; i++) {
			...
			wsprintf(szKeyName, _T("%s"), szBuf);

			GetPrivateProfileString(szSection, szKeyName, _T("Normal"), szType, sizeof(szType) / sizeof(_TCHAR), szFullPathName);

			if ((strcmp(szType, _T("Normal")) == 0) || (strcmp(szType, _T("Ex")) == 0) || (strcmp(szType, _T("Sp")) == 0)) {
				ZeroMemory(&item, sizeof(LVITEM)); 
				item.iItem = i;//行
				item.mask = LVIF_TEXT;
				item.iSubItem = 4;//列:Type
				if (strcmp(szType, _T("Normal")) == 0) {
...
int List_WriteNPC(HWND hWnd,HWND hList)
{
	...
	if (g_ListAreaID >= 0x0100) {
		//セクション名を作成
		wsprintf(szSection, _T("Area%04X"), g_ListAreaID);
		//フィルターを作成

		ZeroMemory(szFilter, sizeof(szFilter));
		if (g_Menu.list_control == 0) {
			//ファイルネーム作成
			wsprintf(szFullPathName, _T("AutoLoad.Area%04X-Name"), g_ListAreaID);
			//\0を_で作成しているため、後で置換
			wsprintf(szFilter, _T("Area%04X File(*.Area%04X)_*.Area%04X_All (*.*)_*.*__"), g_ListAreaID, g_ListAreaID, g_ListAreaID);
		}
		else {
			//ファイルネーム作成
			wsprintf(szFullPathName, _T("AutoLoad.Area%04X"), g_ListAreaID);
			//\0を_で作成しているため、後で置換
			wsprintf(szFilter, _T("Area%04X File(*.Area%04X-Name)_*.Area%04X-Name_All (*.*)_*.*__"), g_ListAreaID, g_ListAreaID, g_ListAreaID);
		}
	}
	else {
		//セクション名を作成
		wsprintf(szSection, _T("Area%02X"), g_ListAreaID);
		//フィルターを作成
		ZeroMemory(szFilter, sizeof(szFilter));
		if (g_Menu.list_control == 0) {
			//ファイルネーム作成
			wsprintf(szFullPathName, _T("AutoLoad.Area%02X"), g_ListAreaID);
			//\0を_で作成しているため、後で置換
			wsprintf(szFilter, _T("Area%02X File(*.Area%02X)_*.Area%02X_All (*.*)_*.*__"), g_ListAreaID, g_ListAreaID, g_ListAreaID);
		}
		else {
			//ファイルネーム作成
			wsprintf(szFullPathName, _T("AutoLoad.Area%02X-Name"), g_ListAreaID);
			//\0を_で作成しているため、後で置換
			wsprintf(szFilter, _T("Area%02X File(*.Area%02X-Name)_*.Area%02X-Name_All (*.*)_*.*__"), g_ListAreaID, g_ListAreaID, g_ListAreaID);
		}
	}
	for (int i = 0; i < sizeof(szFilter); i++) {
		if (szFilter[i] == _T('_')) {
	...
	if(GetSaveFileName(&ofn) == 0){//失敗時
		...
		if (g_Menu.list_control == 0) {
			...
			for (int i = 0; i < nItemCount; i++) {
				...
				wsprintf(szKeyName, _T("NOTE%s"), szBuf);
				//NOTE
