...
DWORD GetDllModuleAddress(int Num,int *pId,HANDLE *polHandle)
{
	...
	//madCHookをインジェクトする為フルアクセス設定
	*polHandle = OpenProcess(PROCESS_ALL_ACCESS,false,pe32.th32ProcessID);
	if(*polHandle == NULL){
		return 0;
	}
...
int GetProcess(PROCESSENTRY32 *pe32,int iProcessCounter)
{
	...
	if(Process32First(hSnapshot,pe32)){
		do{
			if(_tcscmp(_T("pol.exe"),pe32->szExeFile) == 0){
				iProcessCounter--;
...
int GetModule(MODULEENTRY32 *mo32,PROCESSENTRY32 pe32)
{
	...
	if(Module32First(hModuleSnapshot,mo32)){
		do{
			if(_tcscmp(_T("FFXiMain.dll"),mo32->szModule) == 0){
				nFlag = 1;
				break;
			}
